...
function calculateAdPrice(membersCount) {
  ...
  const maxPrice = 650; // Максимальная цена
...
bot.command("info", privateChatMiddleware, isAdminAndModeratorMiddleware, async (ctx) => {
  ...
  ⚙️ *Модерация:* ${db.moderate === "on" ? "Включено" : (state === "off") ? "Выключено" : "Тестирование" || "Не указана"}
...
_/moderate test - посмотреть, как бот проверяет входящие сообщение из группы(работает после обучения нейросети)_
    ...
    try {
      ...
      if (action === "approve" || action === "reject") {
        ...
        if (trainingCount >= trainingGoal) {
          ...
          await ctx.reply(
            "🧠 Обучение завершено. Нейросеть теперь может модерировать автономно. Чтобы запустить модерацию, введи /moderate on",
          );
